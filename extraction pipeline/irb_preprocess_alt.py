#!/usr/bin/env python3
"""
irb_preprocess_alt.py -- Alt preprocessing: groups 3 consecutive windows into
...
OUTPUT:
  preprocessed_alt/  -- same HDF5 schema as preprocessed/, different pad_size
  Gravity shape stored: (270, 3) instead of (90, 3). irb_extract.py handles both
  because it interpolates x_gravity to TARGET_GRAV_LEN=300 regardless of T.

COMPATIBILITY:
  Output HDF5 schema is identical to irb_preprocess.py. irb_extract.py,
  verify_embeddings.py, and export_legacy_schema.py all work unchanged.

Usage:
    export BIOPM_ROOT=CS690TR
    python irb_preprocess_alt.py --output preprocessed_alt/
    python irb_preprocess_alt.py --output preprocessed_alt/ --subject 549
"""
...
# pad_size scales linearly with window duration: int(WS * 192 / 10)
# 3s -> 57, 9s -> int(9 * 192 / 10) = 172. Use 171 (3 * 57) for clean math.
CONFIG = {
    "HighF1": 12, "LowF1": 0.5, "Order1": 6,
    "target_FS": 30,
    "WS": 9,
    "pad_size": 171,        # GROUP_SIZE * 57 = 3 * 57
...
def main():
    p = argparse.ArgumentParser(
        description="Alt Bio-PM preprocessing: 9-second grouped blocks"
    )
    p.add_argument("--output",    required=True, help="Output dir for HDF5 files")
    ...
    print("=" * 64)
    print("Bio-PM IRB Alt Preprocessing (grouped windows)")
    print("=" * 64)
    ...
    print()
    print(f"Next: python irb_extract.py --preprocessed {args.output} \\")
    print(f"        --output features/biopm_features_alt.npz")
    print("=" * 64)
