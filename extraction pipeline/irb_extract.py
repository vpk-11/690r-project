#!/usr/bin/env python3
"""
irb_extract.py -- Extract Bio-PM embeddings from preprocessed IRB HDF5 files.
...
def main():
    os.chdir(REPO_ROOT)
    p = argparse.ArgumentParser(description="Extract Bio-PM features from IRB data")
    ...
    p.add_argument("--output",       default="features/biopm_features.npz")
    p.add_argument("--batch_size",   type=int, default=32)
    p.add_argument("--device",       default="cpu", choices=["cpu", "cuda", "mps"])
    args = p.parse_args()

    print("=" * 64)
    print("Bio-PM IRB Feature Extraction")
    print("=" * 64)
    ...
    print(f"  Use features[:, 128:] for gravity-only if transformer stream is zero")
    print("=" * 64)
