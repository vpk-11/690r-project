...
def masked_mean_std_nanpad(tokens, valid_mask):
    ...
    for b in range(B):
        ...
        if n == 0:
            continue
        real_tokens = tokens[b, v]
        out_mean[b] = real_tokens.mean(dim=0)
        if n > 1:
            out_std[b] = real_tokens.std(dim=0, unbiased=False)

    return torch.cat([out_mean, out_std], dim=-1)
...
def load_all_h5(data_root):
    ...
    X_grav = np.concatenate(x_grav_list)
    X_raw = np.concatenate(raw_list)
    ...
    acc_patches = X_acc[:, :, :NORM_SIZE]
    pos_info = X_acc[:, :, NORM_SIZE]
    add_embedding = X_acc[:, :, NORM_SIZE + 1:]

    return (acc_patches, pos_info, add_embedding,
            X_grav, X_raw, labels, subj, week, ARAT, FMA)


def run_extraction(preprocessed_dir, checkpoint_path, batch_size=32, device="cpu"):
    print("Loading HDF5 files ...")
    (acc_patches, pos_info, add_embedding,
     X_grav, _, labels, subj, week, ARAT, FMA) = load_all_h5(preprocessed_dir)
    ...
    valid_tok = ~np.isnan(acc_patches[:, :, 0])
    pct_any_valid = 100 * valid_tok.any(axis=1).mean()
    print(f"  Windows with >=1 valid token: {pct_any_valid:.1f}%")
    ...
    valid_t = torch.from_numpy(valid_tok).bool()
    grav_t = torch.from_numpy(X_grav).float()
    lbl_t = torch.from_numpy(labels).float()

    dataset = TensorDataset(acc_t, pos_t, add_t, valid_t, lbl_t)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)

    all_features = []
    global_idx = 0

    print(f"\nRunning encoder (device={device}, {N} windows, {len(loader)} batches) ...")
    with torch.no_grad():
        for batch_acc, batch_pos, batch_add, batch_valid, _ in tqdm(loader, desc="Extracting", unit="batch", dynamic_ncols=True):
            ...
            if g.shape[-1] != TARGET_GRAV_LEN:
                g = F.interpolate(g, size=TARGET_GRAV_LEN, mode="linear", align_corners=False)
            g_flat = g.reshape(bs, -1)
    ...
    print(f"  Transformer [0:128]  mean_abs={np.abs(xfm_stream).mean():.4f}  std={xfm_stream.std():.4f}")
    print(f"  Gravity     [128:]   mean_abs={np.abs(grav_stream).mean():.4f}  std={grav_stream.std():.4f}")
