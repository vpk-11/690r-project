#!/usr/bin/env python3
"""
export_legacy_schema.py -- Build old-style visit-level schema from BIOPM window-level output.

Input (from irb_extract.py):
  features/biopm_features.npz with keys:
    features, labels, subj, week, ARAT, FMA

Output (legacy-compatible):
  features/biopm_features_legacy_schema.npz with keys:
    features, features_even, features_odd, feature_names,
    labels, pids, arat, fma, subjects, weeks

Notes:
  - Output vectors remain 1028-d BIOPM vectors.
  - Labels keep current convention: 1=healthy, 0=stroke.
"""
...
def even_odd_split(subj_arr, week_arr):
    """Within each (subj, week), even positions -> even split, odd positions -> odd split."""
...
def build_legacy_schema(source_npz, out_npz):
    ...
    # Enforce visit alignment between full/even/odd outputs
    ...
    print("=" * 64)
    print("Legacy Schema Export Complete")
    print("=" * 64)
...
def main():
    p = argparse.ArgumentParser(description="Export BIOPM legacy-compatible visit-level schema")
    p.add_argument("--source", default="features/biopm_features.npz")
    p.add_argument("--output", default="features/biopm_features_legacy_schema.npz")
    args = p.parse_args()
